		...
		do {
			if(type == "Admin") {
				...
				System.out.println("\nTo check customer info choose '1'\n"
						+ "To create a user choose '2'\n"
						+ "To create a new account choose '3'\n"
						+ "To perform functions on accounts choose '4'\n"
						+ "To cancel an account choose '5'\n"
						+ "To exit choose '6'");
				...
				case 1:
					...
					try {
						...
						System.out.println(accounts.getAccountBalance(customers.getAccountId(accountNum)));
					} catch (SQLException e) {
						e.printStackTrace();
					}
					break;
				case 2:
					...
					try {
						...
						users.addNewUser(username1, password);
						System.out.println("User created successfully!");
						System.out.println("User id is " + users.getUserIdFromUsername(username1));
				...
				case 3:
					...
					try {
						...
						existingUsers = (ArrayList<User>) newUsersPull.getAllUsers();
					} catch (SQLException e) {
						e.printStackTrace();
					}
					...
					if(empty) {
						...
						do {
							...
							for(int i=0; i < existingUsers.size(); i++) {
								if(existingUsers.get(i).getUserId() != userId) {
									invalidUser = true;
							...
							System.out.println("Will you add or remove this account? \n"
									+ "'1' for add\n"
									+ "'2' for remove");
							int choice1 = Integer.parseInt(input.next());
							if(choice1 == 1) {
				...
				case 4:
					...
					do {
						System.out.println("\nTo check the balance choose '1'\n"
								...
								+ "To return to the previous menu choose '5'");
				...
				case 5:
					...
					System.out.println("Enter the userId number: ");
				...
				int accountId = 0;
				try {
					accountId = customers.getAccountId(users.getUserIdFromUsername(username));
				} catch (SQLException e1) {
					// TODO Auto-generated catch block
					e1.printStackTrace();
				}
				if(accountId == 0) {
					System.out.println("You currently don't have an account, \n"
							+ "please create one to continue\n");
					System.out.println("To make a an account choose '1'\n"
							+ "To exit choose '2'");
					choice = Integer.parseInt(input.next());
					switch(choice) {
				...
				System.out.println("\nTo check your balance choose '1'\n"
						...
						+ "To make a withdrawal choose '3'\n"
						+ "To make a transfer choose '4'\n"
						+ "To delete an empty account choose '5'\n"
						+ "To exit choose '6'");
				...
				switch(choice) {
					...
					case 5:
						...
						try {
							...
							hasBalance = accounts.getAccountBalance(customers.getAccountId(singleUser.getUserIdFromUsername(username)));
						...
						if(hasBalance > 0) {
							System.out.println("This account has an active balance, are you sure you want to delete?\n"
									+ "'1' for yes\n"
									+ "'2' for no");
							int sameChoice = input.nextInt();
							switch(sameChoice) {
								case 1:
									...
									try {
										...
										accounts.deleteAccount(customers.getAccountId(singleUser.getUserIdFromUsername(username)));
										customers.deleteCustomer(singleUser.getUserIdFromUsername(username));
							...
							System.out.println("Empty account deleted!");
						}
						break;
					case 6:
						start(users);
